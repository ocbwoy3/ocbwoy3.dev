...
export function onWSEvent(f:(e:{t:SocketRecvMessageType,c:AnySocketEvent})=>any) {
	REMGlobalEventsWS.on("recv",f)
}

export function onWSLog(f:(a: [string,string])=>any) {
	REMGlobalEventsWS.on("wslog",f)
}

export function wsLog(t:string,author:string="SOCKET") {
	REMGlobalEventsWS.emit("wslog",[author,t])
}