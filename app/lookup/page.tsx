	...
	return (
		<div className="block">
			...
			<span className="inline-flex gap-x-2">
				<div className="w-2"/>
				<Input placeholder="Roblox Username" ref={inputRef}/>
				<Button onClick={()=>{
					console.log(inputRef.current.value);
					setIsLoading(true)
					getBanInformation(inputRef.current.value).then(a=>{
						setBanInfo(a);
						setIsLoading(false);
					}).catch(()=>{
						setBanInfo(null);
								...
								<>
									...
									<Alert className="inline-flex">
										...
										<div>
											...
											<AlertDescription className="text-muted-foreground inline-flex">
												<Badge variant="outline">User ID: {banInfo.userId}</Badge>
											</AlertDescription>
										</div>
									</Alert>
									<br/>
										...
										banInfo.bans.map(ban=>(
											<Alert key={ban.banProvider+ban.banProviderIcon} className="inline-flex">
												...
												<div>
													...
													<AlertDescription>{ban.reason}</AlertDescription>
													<AlertDescription className="text-muted-foreground inline-flex">
														{ban.permBan ? (<><Badge variant="outline">Banned Forever</Badge><div className="w-1"/></>) : ""}
														{ban.crashBan ? (<><Badge variant="outline">Crash Ban</Badge><div className="w-1"/></>) : ""}
														<Badge variant="outline">Moderator: {ban.moderator}</Badge>
													</AlertDescription>
												</div>
											</Alert>
