...
export const DummyUserInfo: RobloxUser = {
	...
	AvatarImageUrl: "/placeholder_pfp.webp",
	isDummyData: true,
...
export async function getUserIdFromName(name:string): Promise<number|null> {
	let __user = await fetch(`https://users.roproxy.com/v1/usernames/users`,{
		method: "POST",
		cache: "no-store",
		body: JSON.stringify({
			usernames: [name],
...
export async function getUserPrimaryGroup(userId:number): Promise<RobloxGroup|null> {
	let __groups = await fetch(`https://groups.roproxy.com/v1/users/${userId.toString()}/groups/roles?includeLocked=false`,{cache: "no-store"})
...
export async function getUserInformation(userId:number): Promise<RobloxUser> {
	let __userInfo = await fetch(`https://users.roproxy.com/v1/users/${userId.toString()}`,{cache: "no-store"})
	let userInfo: RobloxUserAPIData = (await __userInfo.json())

	let __userThumb = await fetch(`https://thumbnails.roproxy.com/v1/users/avatar-bust?userIds=${userId.toString()}&size=420x420&format=Png&isCircular=false`,{cache: "no-store"})
